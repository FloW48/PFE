  ...
  export default {
    ...
    methods:{
        ...
        submit(){
            ...
            let data = {id:this.id,prenom:this.prenom, nom:this.nom, surnom:this.surnom, email:this.email, statut:this.statut}

            axios
                .post("http://localhost:3000/editEnseignant", data, {
                    headers:{
    ...
    created: function(){
        ...
        if(this.$store.state.enseignantToModify){
            let enseignant = this.$store.state.enseignantToModify
            this.id = enseignant._id
